...
DrawSomething AI Platform 是一个基于 AI 的绘画挑战游戏平台。该平台让玩家专注于绘画创作，而 AI 负责智能识别绘画内容，提供全新的游戏体验。

### 📑 快速导航

- **🚀 部署** → [快速开始](#快速开始) | [生产部署](#生产部署选项) | [Docker](#docker-部署推荐)
- **⚙️ 配置** → [环境变量](#环境变量配置) | [数据库](#数据库配置) | [AI 服务](#重要ai-服务配置)
- **🎮 使用** → [功能模块](#功能模块) | [游戏玩法](#游戏玩法) | [API](#api-文档)

### ⚡ 快速开始

**首次使用？** 查看 [AI 配置快速指南](docs/AI_CONFIG_GUIDE.md) 3 分钟快速上手！

### ✨ 项目特色

- **🤖 AI 驱动的图像识别**：集成多模态大模型，自动识别绘画内容
- **🎨 专业画板工具**：支持压力感画笔、颜色选择、画笔粗细调节
- **📱 移动端优化体验**：专为移动设备和触屏优化的界面
- **🛡️ 管理员管理**：提供管理员登录和画廊内容管理功能
- **🎯 绘画闯关**：挑战不同难度的关卡，支持自定义关卡创建
...
cd backend
pip install -r requirements.txt
export MODEL_KEY="your_production_key"  # 可选
...
**本项目不提供本地内置大模型推理**。所有 AI 功能均需通过配置外部云 AI 服务来实现。

#### 为什么不内置大模型？

- 即使是 256M 参数的轻量级模型，在普通 CPU 上的推理速度也**不合理**（30+ 秒/次）
- 用户体验无法接受
- 优先级推荐使用**云服务 API**获得更好的性能和准确度

#### ✅ 推荐配置方案

**免费方案：百度飞桨 AI Studio**（推荐）
- 视觉模型：`ernie-4.5-vl-28b-a3b`（"绘画猜词"功能需要）
- 文生图模型：`Stable-Diffusion-XL`（"AI 画你猜"功能需要）
- 免费额度：每月 100 万 tokens
- API 地址：`https://aistudio.baidu.com/llm/lmapi/v3`
- 获取方式：[注册百度 AI Studio](https://aistudio.baidu.com/account/accessToken)

**其他云服务选项**
- OpenAI GPT-4V（需付费）
- Claude Vision（需付费）
- 任何支持 OpenAI 兼容格式的 API

#### 🔧 配置步骤

1. **启动应用后，主页会自动检测 AI 配置**
   - 如果未配置，会显示弹窗提示
   - 点击"去配置"按钮进入配置页面

2. **在 AI 配置页面填写信息**
   - 选择"自定义服务"标签页
   - 填入 API URL、API Key、模型名称
   - 点击"测试视觉模型"或"测试文生图模型"验证配置
...
访问 `http://localhost:8080`，使用以下凭证登录：
- 系统：PostgreSQL
...
# Windows：从 https://www.postgresql.org/download/windows/ 下载安装
```
...
```bash
# 最常用的命令
alembic upgrade head          # 应用所有待处理迁移（生产部署必需）
alembic current              # 查看当前版本
alembic history              # 查看迁移历史
alembic downgrade -1         # 回滚一步（谨慎使用）
alembic revision --autogenerate -m "描述"  # 生成新迁移文件
```

> **详细命令说明**：更多 Alembic 命令请参考 [Alembic 官方文档](https://alembic.sqlalchemy.org/)或后续扩展文档
...
> **登录提示**：绘画闯关需要用户登录才能保存进度和使用AI识别功能。未登录用户可以体验绘画功能，但无法使用AI猜词。
...
| `/app/level-set` | 绘画闯关 | 挑战预设和自定义关卡 |
| `/app/level-config` | 关卡配置 | 创建和管理自定义关卡 |
| `/app/draw` | 自由绘画 | 无限制的自由创作模式 |
| `/app/challenge-draw` | 挑战绘画 | 绘画闯关绘画页面 |
